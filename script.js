  class MusicPlayer {
      ...
      handleFileSelect(e) {
        ...
        this.addFiles(files);
        this.fileInput.value = ''; // Limpiar para permitir recarga
      }
      ...
      updatePlaylist() {
        ...
        this.playlistEl.innerHTML = this.playlist.map((song, index) => {
          let albumArtHtml = '<div class="song-icon">🎵</div>';
          ...
          return `
            <div class="song-item ${index === this.currentIndex ? 'playing' : ''}" data-index="${index}">
              ...
              <div class="song-actions">
                <button class="action-btn" onclick="player.showSongMetadata(${index})" title="Ver información">ℹ️</button>
                <button class="action-btn" onclick="player.removeSong(${index})" title="Eliminar">🗑️</button>
              </div>
            </div>
          `;
        }).join('');
